...
public:
    int ladderLength(string beginWord, string endWord, vector<string>& wordList) {
        ...
        unordered_map<string, int> mp; //maps word in list to index
        for(int i = 0; i < n; i++)
        {
            mp[wordList[i]] = i;
        }
        ...
        for(int i = 0; i < m; i++) //generate one char modification from beginWord
        {
            for(char c = 'a'; c <= 'z'; c++)
            {
                if(c == beginWord[i]) continue;
                string word = beginWord.substr(0, i) + c + beginWord.substr(i + 1);
                if(mp.count(word) && !visit.count(mp[word]))
                {
                    q.push(mp[word]);

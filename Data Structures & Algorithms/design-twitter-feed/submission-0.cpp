...
public:
    int count = 0;
    unordered_map<int, unordered_set<int>> followMap; //maps a user to its followers (both are ids)
    unordered_map<int, vector<pair<int, int>>> tweetMap; //maps a user to tweets + count (both are ids)
    Twitter() {
        count = 0;
    }
